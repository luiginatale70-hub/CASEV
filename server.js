...
app.use((req, res, next) => {
  ...
  if (req.session && req.session.user) {
    ...
    if (ruolo === 'admin' || ruolo === 'istruttore') {
      res.locals.esamiUrl = '/esami/instructor';
    } else {
      res.locals.esamiUrl = '/esami';
    }
  } else {
    res.locals.esamiUrl = '/esami';
  }
...
app.use('/admin/pubblicazioni-path', require('./modules/pubblicazioni/admin_pubblicazioni'));
