...
router.use(requireAnyRole('istruttore','admin','admin_esami'));
...
router.post('/questions/:bankId(\\d+)/delete',requireAnyRole('admin','admin_esami'),async(req,res)=>{
...
router.get('/questions/import',requireAnyRole('admin','admin_esami'),(req,res)=>
  res.render('instructor/questions_import',{title:'Importa domande'}));

router.post('/questions/import',requireAnyRole('admin','admin_esami'),upload.single('file'),
  body('exam_type').trim().notEmpty(),
  async(req,res)=>{
    if(!validationResult(req).isEmpty()){req.flash('error','Tipologia esame obbligatoria.');return res.redirect('/instructor/questions/import');}
    if(!req.file){req.flash('error','Seleziona un file Excel.');return res.redirect('/instructor/questions/import');}
...
router.get('/exams/assign',async(req,res)=>{
  const students=await all('SELECT s.id,s.'+BT+'rank'+BT+',s.name,s.surname,s.email,c.name as class_name,c.command,c.category FROM esami_students s LEFT JOIN esami_classes c ON c.id=s.class_id ORDER BY s.surname,s.name');
...
router.post('/exams/assign',
  ...
  async(req,res)=>{
    if(!validationResult(req).isEmpty()){req.flash('error','Dati non validi.');return res.redirect('/instructor/exams/assign');}
    const student=await get('SELECT * FROM esami_students WHERE id=?',[req.body.student_id]);
    if(!student){req.flash('error','Allievo non trovato.');return res.redirect('/instructor/exams/assign');}
    ...
    const bankIds=(await all('SELECT id FROM esami_question_banks WHERE exam_type=?',[examType])).map(r=>r.id);
    if(!bankIds.length){req.flash('error','Nessun questionario per questa tipologia.');return res.redirect('/instructor/exams/assign');}
    const ph=bankIds.map(()=>'?').join(',');
    let picked=[];
    if(topicsCounts.length){
      const req_tot=topicsCounts.reduce((a,x)=>a+x.count,0);
      if(req_tot!==num){req.flash('error','Distribuzione non valida: somma='+req_tot+', richieste='+num);return res.redirect('/instructor/exams/assign');}
      for(const t of topicsCounts){const row=await get('SELECT COUNT(*) as cnt FROM esami_questions WHERE bank_id IN ('+ph+') AND topic=?',[...bankIds,t.topic]);if((row?row.cnt:0)<t.count){req.flash('error','Domande insufficienti per "'+t.topic+'"');return res.redirect('/instructor/exams/assign');}}
    ...
    if(picked.length<num){req.flash('error','Domande insufficienti: trovate '+picked.length+', richieste '+num);return res.redirect('/instructor/exams/assign');}
    ...
    sendMail({to:student.email,subject:'Portale NAAF - Esame assegnato',html:'<p>Esame assegnato: '+examType+', '+num+' domande, '+duration+' min.</p>'}).catch(()=>{});
    req.flash('success','Esame assegnato (ID '+ex.lastID+').');
...
router.post('/exams/:id/delete',async(req,res)=>{
  ...
  if(!exam){req.flash('error','Esame non trovato.');return res.redirect('/instructor/exams');}
  if(exam.status!=='ASSEGNATO'){req.flash('error','Non puoi eliminare un esame già svolto.');return res.redirect('/instructor/exams');}
...
router.get('/exams/:id/report.pdf',async(req,res)=>{
  ...
  doc.fontSize(11).text('ID: '+exam.id).text('Allievo: '+exam.rank+' '+exam.surname+' '+exam.name).text('Tipologia: '+exam.exam_type).text('Risultato: '+(exam.score_percent!=null?Number(exam.score_percent).toFixed(1)+'%':'-')).moveDown();
